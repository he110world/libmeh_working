...
vec4 normal_mapping(vec2 texcoord )
{
...
//	vec4 color = texture2D(color_map, texcoord);
	...
	// normal map
	vec3 normal = texture2DGrad(relief_map, texcoord, dx, dy).xyz;
...
/*
  When we draw wall brick, for each pixel, we have to know the ray distance. how do we know that?
  We can work out some fomulers for cube, but for general polyhedra we can't.

  We know the distance from eye to the front face of the brick, and distance from the back face of it by 
  some shader in a seperate pass --

  distance shader:
  draw back face;
  draw front face;
  subtract the distance => ray distance

  ray casting shader:
  draw front face;
  use the distance value in frame buffer as texture;


  Or compute the distance on CPU by some clipping algorithm and pass the distance as vertex attribute => good

  bricks DON'T intersect -- if they do, they should clip with each other.
...
//	if(p.z > 0.5) result=vec4(1.0);	
	...
	if (count>100) result=vec4(1.0,1.0,1.0,1.0);
